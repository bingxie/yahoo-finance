...
module YahooFinance
  ...
  class Client
    ...
    def quotes(symbols_array, columns_array = %i[symbol last_trade_price last_trade_date change previous_close], options = {})
      # remove invalid keys
      columns_array.select! { |c| COLUMNS.key?(c) }
    ...
    def quote(symbol, columns_array = %i[symbol last_trade_price last_trade_date change previous_close], options = {})
      options[:raw] ||= true
    ...
    def historical_quotes(symbol, options = {})
      ...
      params = {
        ...
        frequency: HISTORICAL_MODES[options[:period]],
        filter: 'history'
      }
    ...
    def read_historical(symbol, url)
      ...
      rows.drop(0).each_with_object([]) do |row, data|
        divs = row.css('td')
        next unless divs[1] && !divs[1].text.include?('Dividend') && !divs[1].text.include?('Stock Split') # Ignore these rows in the table
